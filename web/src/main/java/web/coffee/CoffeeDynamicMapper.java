...
public interface CoffeeDynamicMapper {
	...
	@Update({"update", "coffees", "set coffeeName = #{coffeeName},", "coffeePrice = #{coffeePrice}",
			"where coffeeId = #{coffeeId}"})
	int update(Coffee coffee);
	
	@Select("select coffeeId, coffeeName, coffeePrice from coffees where coffeeId = #{coffeeId}")
